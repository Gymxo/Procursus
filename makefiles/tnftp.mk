...
TNFTPD_VERSION := 20200704
DEB_TNFTP_V    ?= $(TNFTP_VERSION)

tnftp-setup: setup
	wget -q -nc -P $(BUILD_SOURCE) https://ftp.netbsd.org/pub/NetBSD/misc/tnftp/tnftp-$(TNFTP_VERSION).tar.gz
	wget -q -nc -P $(BUILD_SOURCE) https://ftp.netbsd.org/pub/NetBSD/misc/tnftp/tnftpd-$(TNFTPD_VERSION).tar.gz
	$(call EXTRACT_TAR,tnftp-$(TNFTP_VERSION).tar.gz,tnftp-$(TNFTP_VERSION),tnftp)
	$(call EXTRACT_TAR,tnftpd-$(TNFTPD_VERSION).tar.gz,tnftpd-$(TNFTPD_VERSION),tnftpd)
...
tnftp: tnftp-setup openssl
	...
	cd $(BUILD_WORK)/tnftpd && ./configure -C \
		$(DEFAULT_CONFIGURE_FLAGS) \
		--with-pam
	+$(MAKE) -C $(BUILD_WORK)/tnftpd
	+$(MAKE) -C $(BUILD_WORK)/tnftpd install \
	DESTDIR=$(BUILD_STAGE)/tnftp
	$(call AFTER_BUILD)
endif
